...
# ─────────────────────────────────────────────
# 1. ARQUITECTURA VISUAL "NOLASCO CAPITAL V9.4"
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 2. MOTOR DE DATOS (ACTUALIZADO CON DATOS TÉCNICOS)
# ─────────────────────────────────────────────
...
# Si el archivo existe pero no tiene las nuevas columnas, lo forzamos (solo para esta actualización)
if os.path.exists(DB_INMUEBLES):
    temp_df = pd.read_csv(DB_INMUEBLES)
    if "m2" not in temp_df.columns:
...
# ── FICHAS TÉCNICAS (CON DATOS DE COMPARACIÓN) ─
elif "Fichas" in menu:
    ...
    with c_f1:
        ...
        <table class="tech-table">
            ...
            <tr><td class="tech-label">Baños</td><td>{f['Baños']}</td></tr>
            <tr><td class="tech-label">Estado</td><td>{f['Estado']}</td></tr>
            <tr><td class="tech-label">Referencia Catastral</td><td>{f['Ref_Catastral'] if f['Ref_Catastral'] else 'No asignada'}</td></tr>
    ...
    with c_f2:
        ...
        <div class="{clase}">
            ...
            Desviación: <b>{desv:.1f}%</b> respecto a mercado.<br><br>
            <span style="font-size:0.9rem; color:var(--slate);">
                Renta Mercado (Simulada): <b>{renta_mer:,.2f}€</b><br>
                Renta por m²: <b>{renta_act/f['m2']:.2f}€/m²</b>
            </span>
            {f'<div style="margin-top:10px; border-top:1px solid rgba(0,0,0,0.1); padding-top:10px;">📉 <b>Lucro Cesante:</b> estás perdiendo <b style="color:var(--crimson);">{perdida_anual:,.2f}€/año</b></div>' if perdida_anual > 0 else ''}
        </div>
        """, unsafe_allow_html=True)
...
# ── DATOS Y PLANTILLAS (IMPORTADOR) ───────────
elif "Datos" in menu:
    st.markdown('<div class="brand-header">Gestión de Datos y Plantillas</div>', unsafe_allow_html=True)
    
    st.markdown('<div class="section-title">1. Edición Maestra de la Cartera</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">2. Importación Masiva (Excel)</div>', unsafe_allow_html=True)
    uploaded_file = st.file_uploader("Sube tu plantilla Excel aquí", type=["xlsx", "csv"])
    if uploaded_file is not None:
        st.success("Archivo detectado. Procesando integración...")
        # Aquí iría la lógica de merge que comentamos

    st.markdown('<div class="section-title">3. Copias de Seguridad</div>', unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1: st.download_button("📥 Descargar Inmuebles", df_inm.to_csv(index=False), "inmuebles.csv", "text/csv")
    with c2: st.download_button("📥 Descargar Movimientos", df_mov.to_csv(index=False), "movimientos.csv", "text/csv")
